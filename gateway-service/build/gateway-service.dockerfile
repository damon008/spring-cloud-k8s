#FROM 10.10.8.100:5000/bitacube/gateway-service
...
RUN mv target/gateway-service.jar gateway-service.jar
RUN rm -rf src
RUN rm pom.xml
CMD ["sh", "-c", "nohup java $JAVA_OPTS -jar gateway-service.jar", "&"]